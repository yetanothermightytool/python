...
def main():
    ...
    parser.add_argument("--vbrserver", default=None, help="VBR server hostname or IP (overrides .env)")
    ...
    parser.add_argument("--username", default=None, help="Veeam REST API username (overrides .env)")
    ...
    # Resolve vbrserver, username and password from CLI args or .env
    vbrserver = args.vbrserver or os.getenv("VBR_SERVER")
    if not vbrserver:
        parser.error("--vbrserver is required (or set VBR_SERVER in .env)")

    username = args.username or os.getenv("VBR_USERNAME")
    if not username:
        parser.error("--username is required (or set VBR_USERNAME in .env)")

    password = os.getenv("VBR_PASSWORD")
    if not password:
        parser.error("VBR_PASSWORD must be set in .env")

    smb_pass = os.getenv("SMB_PASSWORD")
    if not smb_pass:
        parser.error("SMB_PASSWORD must be set in .env")

    api_url  = f"https://{vbrserver}:9419"
