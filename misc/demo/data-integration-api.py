...
# Script variables - Adjust hostname
...
def run_iscsi_scan(mount_id, session_info, host2scan):
   ...
   for dev in new_disks:
       for part in range(1, 5):
           ...
           try:
               ...
               continue
   # Here you can run anything against the mounted volumes
   print(f"[{host2scan}] 🔍 Simulating scan on mounted volumes...")
   log_message(host2scan, "Simulating scan on mounted volumes...")
   time.sleep(10)
...
def do_mount_scan(token, scanhost, local_ip, restore_point_id, host_name, use_iscsi):
   mount_body = {
   ...
   if use_iscsi:
       run_iscsi_scan(mount_id, mount_info.get("info", {}), host_name)
   else:
       # Here you can run anything against the mounted volumes (FUSE)
       print(f"[{host_name}] ⏳ Simulating scan...")
       log_message(host_name, "Simulating scan...")
       time.sleep(10)
...
def main():
   ...
   # Valid Restore Points platforms for repo2scan function
           ...
           with concurrent.futures.ThreadPoolExecutor(max_workers=args.maxhosts) as executor:
               ...
               for host in hostnames:
                   ...
                   if rp.get("data"):
                       ...
                       futures.append(executor.submit(do_mount_scan, token, scanhost, local_ip, restore_id, host, args.iscsi))
   ...
   do_mount_scan(token, scanhost, local_ip, restore_point_id, args.host2scan, args.iscsi)
   return token
