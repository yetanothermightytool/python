...
# Get password (Fernet) for REST API calls against Veeam Backup & Replication REST API
def get_password():
...
# Run iSCSI Dummy Scanner Function - Presented Volumes are mounted
def run_iscsi_scan(mount_id, session_info, host2scan, restore_point_id):
   ...
   print(f"[{host2scan}|{restore_point_id}] 🔌 iSCSI login to {ip}:{port}")
   ...
   if not new_disks:
       print(f"[{host2scan}|{restore_point_id}] ❌ No new iSCSI disks found.")
       return
       ...
       for part in range(1, 5):
           ...
           try:
               ...
               if fs_type in ("ntfs", "xfs", "ext4"):
                   ...
                   print(f"[{host2scan}|{restore_point_id}] ✅ Mounted {dev_path} -> {mnt_path}")
                   mounted_paths.append(mnt_path)
           except Exception as e:
               print(f"[{host2scan}|{restore_point_id}] ⚠️ Skipping {dev_path}: {e}")
               continue

   print(f"[{host2scan}|{restore_point_id}] 🔍 Simulating scan on mounted volumes...")
   log_message(host2scan, f"[{restore_point_id}] Simulating scan on mounted volumes...")
   time.sleep(10)

   print(f"[{host2scan}|{restore_point_id}] 🧹 Cleaning up...")
...
def do_mount_scan(api_url, token, scanhost, local_ip, restore_point_id, host_name, use_iscsi):
   mount_body = {
       "restorePointId": restore_point_id,
       "type": "ISCSITarget" if use_iscsi else "FUSELinuxMount",
       "targetServerName": scanhost,
       "targetServerCredentialsId": get_veeam_rest_api(
           api_url, "v1/backupInfrastructure/managedServers", token, params={"nameFilter": scanhost}
       )["data"][0]["credentialsId"],
       "allowedIps": [local_ip]
   }
   print(f"[{host_name}|{restore_point_id}] 📦 Publishing disk content for {host_name}...")
   ...
   try:
       ...
       if e.response.status_code == 404:
           print(f"[{host_name}|{restore_point_id}] ⚠️ OS not supported or mount failed. Skipping.")
           log_message(host_name, f"[{restore_point_id}] Mount failed (404), unsupported OS?")
           return
       raise

   if use_iscsi:
       run_iscsi_scan(mount_id, mount_info.get("info", {}), host_name, restore_point_id)
   else:
       print(f"[{host_name}|{restore_point_id}] 🔍 Simulating scan...")
       log_message(host_name, f"[{restore_point_id}] Simulating scan...")
       time.sleep(10)
...
def main():
   ...
   parser.add_argument("--vbrserver", default="https://vbr-host01",
                       help="VBR base URL incl. port (default: https://vbr-host01:9419)")
   parser.add_argument("--latest", action="store_true",
                       help="Automatically use latest restore point (skip selection menu)")
   ...
   # If repo2scan is passed as parameter
   if args.repo2scan:
       ...
       # Go through all restore points on repository
       if args.all:
           ...
           with concurrent.futures.ThreadPoolExecutor(max_workers=args.maxhosts) as executor:
               ...
               for host in hostnames:
                   ...
                   if rp.get("data"):
                       ...
                       futures.append(executor.submit(do_mount_scan, api_url, token, scanhost, local_ip,
                                                      restore_id, host, args.iscsi))
       ...
       # (Repo-Mode Auswahl bleibt wie vorher)
   ...
   do_mount_scan(api_url, token, scanhost, local_ip, restore_point_id, args.host2scan, args.iscsi)
   return token
