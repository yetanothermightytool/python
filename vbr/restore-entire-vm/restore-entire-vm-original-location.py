...
def main():
   ...
   # Construct payload for restore
        ...
        print("Restore VM...")
        restore_vm_uri = "v1/restore/vmRestore/vSphere"
        restore_vm_url = f"{api_url}/api/{restore_vm_uri}"
        vm_recovery = requests.post(restore_vm_url, json=payload, headers=headers, verify=False)

        print("Restore process response:", vm_recovery)
    else:
